...
PathPlanner::PathPlanner() : it_(nh_) {
  ...
  has_ended_pub_ =
      nh_.advertise<std_msgs::Bool>(PATHPLANNER_HAS_ENDED_TOPIC, 1);
...
void PathPlanner::run() {
  ...
  if (!run_node_) {
    cv::Mat path_map = generatePathImg(occupancy_map_, drone_cell_, goal_cell_,
                                       current_path_, ref_waypoints_);
    sendMap(path_map);
    return;
  }
...
void PathPlanner::prepareNewAttempt() {
  ...
  cv::circle(occupancy_map_, goal_cell_, goal_zone_clean_radious, cv::Scalar(0),
             -1);
}
...
void PathPlanner::generateOccupancyMap() {
  ...
  dist_normalized_map =
      distance_map /
      security_distance_th_; // normalize distance map respect to the max dist
  dist_normalized_map.setTo(1.0f,
                            dist_normalized_map > 1.0f); // clip image to 1.0

  // Generate Occupancy Map: MAKE CONVOLUTION TO SOLVE THIS AND EVERYBODY WILL
  // BE HAPPY
  ...
  cv::Range r1(
      cv::Range(std::max(0, drone_px.x - crop_distance),
                std::min(drone_px.x + crop_distance, img_size_.height - 1)));
  cv::Range r2(
      cv::Range(std::max(0, drone_px.y - crop_distance),
                std::min(drone_px.y + crop_distance, img_size_.width - 1)));
  ...
  // cv::Mat ego_map = binary_map(cv::Rect(drone_px.x - crop_distance,
  // drone_px.y - crop_distance,
  //                                       2 * crop_distance, 2 *
  //                                       crop_distance));
  // cv::Mat ego_distance_map = binary_distance_map(cv::Rect(drone_px.x -
  // crop_distance, drone_px.y - crop_distance,
  //                                                         2 * crop_distance,
  //                                                         2 *
  //                                                         crop_distance));
  // cv::Mat ego_occupancy_map = binary_occupancy_map(cv::Rect(drone_px.x -
  // crop_distance, drone_px.y - crop_distance,
  //                                                           2 *
  //                                                           crop_distance, 2
  //                                                           * crop_distance));
...
cv::Mat PathPlanner::filterLaserMap(cv::Mat &mat) {
  if (!prev_mat_ptr) {
    prev_mat_ptr = std::make_shared<cv::Mat>();
  ...
  cv::multiply(combined_mask, cv::Scalar(decrement_per_free * maximum_value),
               combined_mask);
  cv::subtract(*prev_mat_ptr, combined_mask, *prev_mat_ptr);
...
bool PathPlanner::controlNodeSrv(std_srvs::SetBool::Request &_request,
  ...
  if (run_node_ == _request.data) {
    ...
    std::cout << "Run node already set to " << std::to_string(run_node_)
              << std::endl;
    return true;
  }
...
cv::Point2i PathPlanner::findNearestAvailableCell(const cv::Point2i &_cell,
...
void PathPlanner::showMap(const cv::Mat &_map, const std::string &_map_name,
  ...
  //   img_h_, img_w_); cv::Mat map_img = generateShowImg(_map,
  //   img_drone_position); cv::imshow(_map_name, map_img);
  // }
  // else
  // {
...
cv::Mat
PathPlanner::generatePathImg(const cv::Mat &_map, const cv::Point2i &_drone_px,
...
cv::Point2f PathPlanner::grid2coord(const cv::Point2i &_point) {
  ...
  if (center_origin_) {
    coord_point.x =
        (double)(grid_size_.height / 2.0 - _point.x) * occmap_resolution_;
    coord_point.y =
        (double)(grid_size_.width / 2.0 - _point.y) * occmap_resolution_;
  } else {
